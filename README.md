...
The Community bot ships as a multi-arch container image on the GitHub Container Registry. The current published pre-alpha release is `v0.5.0`:
...
The `v0.5.0` image is published for pre-alpha validation. It is not yet the
complete self-host Community product; the `v0.6.0` sprint is focused on
observability (OpenTelemetry + a `/metrics` endpoint) and supply-chain
hardening. The bot is distributed as:
...
- `vX.Y.Z` — pinned to the exact SemVer release (e.g. `v0.5.0`).
- `vX.Y` — moving alias for the latest patch of a minor (e.g. `v0.5`).
