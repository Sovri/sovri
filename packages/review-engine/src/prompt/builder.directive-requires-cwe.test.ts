...
// security or bug finding tied to a known weakness — not only the soft "omit otherwise" escape hatch.
// The positive instruction must scope the omission to non-security/bug findings (style, performance),
// so the model is biased to emit a CWE exactly where compliance enrichment can fire.

import { describe, expect, it } from "vitest";

import { buildSystemPrompt } from "./builder.js";

// The directive carries a positive obligation, applied to every security/bug weakness, not a soft
// "for a finding, set cwe ... omit otherwise". This phrase is the load-bearing positive instruction.
const POSITIVE_CWE_INSTRUCTION =
  "On every security or bug finding tied to a known weakness, set `cwe`";

describe("R-02: the CWE directive tells the model to attach a CWE to security and bug findings", () => {
  ...
  it("a non-security and non-bug finding is not forced to carry a CWE", () => {
    // When the review engine builds the system prompt for the "full" mode
    const systemPrompt = buildSystemPrompt({ mode: "full" });

    // Then the CWE directive tells the model to omit the CWE on a style or performance finding
    expect(systemPrompt).toContain("omit `cwe`");
    expect(systemPrompt.toLowerCase()).toContain("style or performance");
  });
});
