...
- `feat(compliance)`: add the internal Ed25519 audit-trail signer (task-97, #1942) —
  ...
  tamper-evident. The assembled entry is validated against `SignedAuditTrailEntrySchema`
  before return, so a first entry (`trail.started`) is forced to carry a null `previous_hash`.
